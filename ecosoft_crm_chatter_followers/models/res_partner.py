...
MODELS_MESSAGES_FORCE = ["crm.lead"]
TYPES_MESSAGES = ["comment", "email"]
...
class Partner(models.Model):
    ...
    def _notify(self, message, rdata, record, force_send=False, send_after_commit=True, model_description=False, mail_auto_delete=True):
        ...
        if message.model in MODELS_MESSAGES_FORCE and message.message_type in TYPES_MESSAGES:
            list_users = [user["id"] for user in rdata if user["type"]=="user" ]            
            recipients["user"]["recipients"] = list_users
