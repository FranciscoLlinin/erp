...
class AccountTax(models.Model):
    ...
    amount_type = fields.Selection(default='percent', string="Tax Computation", required=True,
                                   selection=[('group', 'Group of Taxes'), ('fixed', 'Fixed'),
                                              ...
                                              ('transferred', 'Transferred Fixed'),
                                              ('withhold', 'Withhold Fixed')],
                                   help="""
        ...
        - Transferred Fixed: Transferred by amount
        - Withhold Fixed: Withhold by amount
            """)

    def _compute_amount(self, base_amount, price_unit, quantity=1.0, product=None, partner=None, line=False):
        """ Returns the amount of a single tax. base_amount is the actual amount on which the tax is applied, which is
            price_unit * quantity eventually affected by previous taxes (if tax is include_base_amount XOR price_include)
        """
        self.ensure_one()

        if self.amount_type == 'transferred' or self.amount_type == 'withhold':
            if line:
                tli = next((x for x in line.tax_local_ids if x.tax_id == self), None)
                if tli:
                    balance_multiplicator = 1 if tli.tax_id.amount_type == 'transferred' else -1
                    return tli.amount * balance_multiplicator
    ...
    def compute_all(self, price_unit, currency=None, quantity=1.0, product=None, partner=None, is_refund=False, handle_price_include=True, include_caba_tags=False, line=False):
            ...
            for tax in reversed(taxes):
                ...
                if tax.price_include or self._context.get('force_price_include'):
                    if tax.amount_type == 'percent':
                        ...
                        tax_amount = tax._compute_amount(base, sign * price_unit, quantity, product, partner, line) * sum_repartition_factor
        ...
        for tax in taxes:
            ...
            if not skip_checkpoint and price_include and total_included_checkpoints.get(i) is not None and sum_repartition_factor != 0:
                ...
                tax_amount = tax.with_context(force_price_include=False)._compute_amount(
                    tax_base_amount, sign * price_unit, quantity, product, partner, line)
