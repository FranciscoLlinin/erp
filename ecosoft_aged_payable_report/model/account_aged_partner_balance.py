...
class ecosoft_report_account_aged_payable(models.AbstractModel):
    _inherit = "account.aged.payable"
    
    def _build_options(self, previous_options=None):
        options = super(ecosoft_report_account_aged_payable, self)._build_options(previous_options=previous_options)
        if not previous_options:
            previous_options = {}
        options['report_type'] = 'payable'
    ...
    def _set_context(self, options):
        ctx = super(ecosoft_report_account_aged_payable, self)._set_context(options)
        
        # Solo se puede seleccionar una moneda o todas las monedas
    ...
    def _get_partner_move_lines_filtered(self, account_type, date_from, target_move, period_length,currency_id):
        ...
        query = '''
            ...
            WHERE (l.account_id = account_account.id)
                ...
                AND ''' + reconciliation_clause + partner_clause + currency_clause + '''
                AND (l.date <= %s)
        ...
        for i in range(5):
            ...
            for line in self.env['account.move.line'].browse(aml_ids).with_context(prefetch_fields=False):
                ...
                if currency_id and currency_id == line.currency_id.id:
                    ...
                    line_amount = line.company_id.currency_id._convert(line.balance, user_currency, user_company,
                                                                       date_from)
        ...
        query = '''SELECT l.id
                ...
                AND l.company_id IN %s
                ORDER BY COALESCE(l.date_maturity, l.date)'''
        cr.execute(query, (tuple(move_state), tuple(account_type), date_from, tuple(partner_ids), date_from,
                           tuple(company_ids)))
        aml_ids = cr.fetchall()
        aml_ids = aml_ids and [x[0] for x in aml_ids] or []
        ...
        for partner in partners:
            ...
            if partner['partner_id']:
                ...
                values['name'] = browsed_partner.name and len(browsed_partner.name) >= 45 and browsed_partner.name[0:40] \
