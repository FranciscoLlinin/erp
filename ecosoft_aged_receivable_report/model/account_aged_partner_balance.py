...
class ecosoft_report_account_aged_receivable(models.AbstractModel):
    _inherit = "account.aged.receivable"
    
    def _build_options(self, previous_options=None):
        options = super(ecosoft_report_account_aged_receivable, self)._build_options(previous_options=previous_options)
        if not previous_options:
            previous_options = {}
        options['report_type'] = 'receivable'
    ...
    def get_currencies(self,previous_options):
        ...
        records = self.env.user.company_id.currency_id + \
            self.env['res.currency'].search([('active', '=', True),
                                             ('id', '!=', self.env.user.company_id.currency_id.id)], order="name")
        res = []
        for c in records:
    ...
    def _set_context(self, options):
        ctx = super(ecosoft_report_account_aged_receivable, self)._set_context(options)
...
class EcosoftReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):            
        ...
        if currency_id:
            return self._get_partner_move_lines_filtered(account_type, date_from, target_move, period_length,
                                                         currency_id)
        else:
            return super(EcosoftReportAgedPartnerBalance, self)._get_partner_move_lines(account_type, date_from,
                                                                                        target_move, period_length)
                
    def _get_partner_move_lines_filtered(self, account_type, date_from, target_move, period_length, currency_id):
        ...
        # Here we add code required for currency filtering
        currency = self.env['res.currency'].with_context(date=date_from)
        ...
        if currency_id:
            ...
            if currency_id == self.env.user.currency_id.id:
                ...
                currency_clause += ' ) '       
            user_currency = currency.browse(currency_id)
        else:
            currency_clause = ''   
        ...
        if currency_id and user_company.account_realization_journal_id:
            journal_clause += ' AND am.journal_id NOT in (%s) ' % user_company.account_realization_journal_id.id
        if currency_id and user_company.invoice_realization_journal_id:
            journal_clause += ' AND am.journal_id NOT in (%s) ' % user_company.invoice_realization_journal_id.id
        ...
        query = '''
            ...
            WHERE (l.account_id = account_account.id)
                ...
                AND ''' + reconciliation_clause + partner_clause + currency_clause + journal_clause +'''
                AND (l.date <= %s)
        ...
        for i in range(5):
            ...
            query = '''SELECT l.id
                    ...
                    WHERE (l.account_id = account_account.id) AND (l.move_id = am.id)
                        ...
                        AND ''' + dates_query + currency_clause + journal_clause +'''
                    AND (l.date <= %s)
                    AND l.company_id IN %s
        ...
        query = '''SELECT l.id
                ...
                WHERE (l.account_id = account_account.id) AND (l.move_id = am.id)
                    ...
                    AND ((l.partner_id IN %s) OR (l.partner_id IS NULL))
                AND (l.date <= %s) ''' + currency_clause + journal_clause + '''
                AND l.company_id IN %s
                ORDER BY COALESCE(l.date_maturity, l.date)'''
...
class EcosoftAccountReport(models.AbstractModel):
    _inherit = 'account.report'
    
    def format_value(self, value, currency=False, blank_if_zero=False):
        if not currency and self.env.context.get('currency_id', False):
            currency = self.env['res.currency'].browse(self.env.context.get('currency_id', False))   
        return super(EcosoftAccountReport, self).format_value(value, currency=currency, blank_if_zero= blank_if_zero)
